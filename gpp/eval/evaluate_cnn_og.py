...
def main():
    parser = argparse.ArgumentParser(description="Compare original CLIP vs CNN-GA-selected patches per image and save visuals.")
    parser.add_argument("--ga_jsonl", type=str, help="Path to GA results JSONL file")
    ...
    for idx in enumerate(dataset):
        ...
        patches = images_to_patches(img)
        patches = torch.cat(patches, dim=0).to(device)

        # logits = model(imgs)
        selected = patch_selector(patches).squeeze(-1) 
        print(f'{selected = }')
